...
const ProfileSummaryCard = (): JSX.Element => (
  <section className="rounded-2xl bg-white p-8 text-center shadow-sm">
    <div className="relative mx-auto h-36 w-36 overflow-hidden rounded-3xl border-4 border-white shadow">
      ...
      <button
        type="button"
        className="absolute bottom-2 right-2 rounded-full bg-[#0b4bd8] p-2 text-white"
      >
    ...
    <button
      type="button"
      className="mt-8 w-full rounded-2xl bg-[#0b4bd8] px-4 py-3 font-semibold text-white"
    >
      Editar Perfil
    </button>
    <button
      type="button"
      className="mt-3 w-full rounded-2xl bg-[#eef3ff] px-4 py-3 font-semibold text-[#0b4bd8]"
    >
      Sair da Conta
    </button>
  </section>
...
const LoyaltyStatusCard = (): JSX.Element => (
  <section className="rounded-2xl bg-[#eef3ff] p-6 shadow-sm">
    ...
    <p className="mt-4 text-sm text-slate-600">
      Você está a 3 cortes de distância de ganhar um serviço completo gratuito.
    </p>
  </section>
);

const BronzeLevelCard = (): JSX.Element => (
  <section className="rounded-2xl bg-[#eef3ff] p-6 shadow-sm">
    <p className="text-sm font-bold uppercase tracking-[0.16em] text-[#0b4bd8]">
      Nível Bronze
    </p>
    <div className="mt-4 h-2 rounded-full bg-[#dce6ff]">
    ...
    <p className="mt-4 text-sm text-slate-600">
      Faltam 3 cortes para você se tornar um cliente Silver e ganhar benefícios.
    </p>
  </section>
);
...
const NotificationSettingsCard = (): JSX.Element => (
  <section>
    ...
    <p className="mt-2 text-slate-500">
      Escolha como você deseja ser lembrado dos seus compromissos.
    </p>
...
const NotificationHistoryCard = (): JSX.Element => (
  <section>
    <div className="mb-4 flex items-end justify-between">
      ...
      <button
        type="button"
        className="font-bold uppercase tracking-[0.2em] text-[#0b4bd8]"
      >
        Limpar tudo
      </button>
    </div>
          ...
          <div className="flex items-start gap-4">
            <div
              className={`rounded-xl p-3 ${
                index === 1
                  ? "bg-[#fff1e6] text-[#ba5a10]"
                  : "bg-[#eef3ff] text-[#0b4bd8]"
              }`}
            >
